...
Historically in global B2B operations (outside of high-density regions like China), consolidation opportunities are often overlooked. Deliveries strictly adhere to the customer's required delivery date, resulting in inefficient Less-than-Container Load (LCL) shipments and wasted truck capacity. *(Note: In regions like China, operations inherently utilize more LCL setups due to decentralized logistics and vast delivery requirements).*
...
| `Customer_ID` | `string` | Unique identifier for the client. |
| `Order_Date` | `datetime` | Timestamp when the order was placed. |
| `Required_Delivery_Date` | `datetime` | SLA agreed delivery date. |
| `Actual_Delivery_Date` | `datetime` | Simulated actual delivery date. |
| `Delay_Days` | `int64` | Number of days delayed past the required date. |
| `Shipping_Cost` | `float64` | Calculated freight cost for the order. |
...
1. **Step-wise Risk Simulation:** An SAP batch job sends the order to the model. The model simulates consolidation possibilities by delaying the delivery of the order for +1, +2, or +3 days, checking the predicted complaint risk at each interval.
2. **Automated Shipment Creation:** 
   - **Safe to Consolidate:** If a delay is safe (Risk < 5%), the API returns a value instructing SAP to automatically create a shipment that bundles the consolidatable deliveries together.
   - **Unsafe to Consolidate:** If a delay is unsafe, the shipment is created individually for each delivery to ensure SLAs are met.

### 🏗️ Proposed System Architecture

```mermaid
graph TD
    A[SAP Batch Job] -->|Order Data Payload| B(ML Consolidation Middleware)
    B --> C{Step-wise Risk Evaluation}
    C -->|Simulate +1 Day Hold| D[Random Forest Predictor]
    D -->|Risk > 5%| E[Reject Hold]
    D -->|Risk <= 5%| F[Accept Hold]
    F -->|Test +2 Days...| C
    E --> G[Determine Consolidation]
    G -->|Safe Delay Found| H[Action: Create Consolidated Shipment]
    G -->|No Safe Delay| I[Action: Create Individual Shipment]
    H --> J[(Enterprise ERP / SAP)]
    I --> J
    J -.-> K[Warehouse Fulfillment]
```
